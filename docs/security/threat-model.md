...
| A06 Vulnerable and Outdated Components | Go modules, npm packages, Docker images, Actions | TODO | CI has govulncheck, npm audit high+ runtime gate, gosec, Trivy, Syft SPDX SBOM generation, Dependabot, and SHA-pinned Actions. Local govulncheck/gosec are clean. | Resolve or accept the moderate PostCSS advisory once Next ships a patched internal dependency; keep Trivy scans and release SBOM publishing in CI. |
